"""Local goal navigation with APF collision avoidance (camera pixel space)."""
...
class FieldBounds:
    ...
    def escape_heading(self, x, y, margin):
        """Heading back toward field center when outside the safe inner rectangle."""
...
def calculate_apf_heading(
    ...
    """Compute steering heading using attractive + repulsive potential fields."""
...
class GoalNavigator:
    ...
    def _neighbor_blocking_forward(self, neighbors):
        ...
        for ox, oy in neighbors:
            ...
            if cos_angle >= cos_limit and dist < blocker_dist:
                blocker_dist = dist
                blocker = (ox, oy)
    ...
    def tick(self, now=None):
        ...
        if distance < self.tolerance:
            if not self._reported_arrival:
                ...
                arrived_seq = self.seq
                self.clear_goal()
                return ("report", "arrived", arrived_seq)
            return ("command", "SS")
        ...
        elif command == "FW" and closest < self.forward_block_distance:
